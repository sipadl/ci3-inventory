...
function exportToExcel2() {
    $("#myTablePrintAble").table2excel({
		// exclude: ".excludeThisClass",
		name: "bahan_baku",
		filename: "bahan_baku.xls", // do include extension
		preserveColors: false // set to true if you want background colors and font colors preserved
	});
}
...
function printDisiniNoteTimbang(val) {
    var prtContent = document.getElementById(`modal-print-2-${val}`);
    console.log(prtContent);
    var WinPrint = window.open('', '', 'left=0,top=10,width=800,height=800,toolbar=0,scrollbars=0,status=0');
    WinPrint.document.write('<html><head><title>pdf</title>');
    // Tambahkan referensi ke file CSS Bootstrap di sini
...
function printDisiniCorrection(val) {
    var prtContent = document.getElementById(`modal-print-correction-${val}`);
    console.log(prtContent);
    var WinPrint = window.open('', '', 'left=0,top=10,width=800,height=800,toolbar=0,scrollbars=0,status=0');
    WinPrint.document.write('<html><head><title>pdf</title>');
    // Tambahkan referensi ke file CSS Bootstrap di sini
...
function printDisiniDataFresh(val) {
    var prtContent = document.getElementById(`modal-print-fresh-${val}`);
    console.log(prtContent);
    var WinPrint = window.open('', '', 'left=0,top=10,width=800,height=800,toolbar=0,scrollbars=0,status=0');
    WinPrint.document.write('<html><head><title>pdf</title>');
    // Tambahkan referensi ke file CSS Bootstrap di sini
...
function printDisiniMemoSubsidi(val) {
    var prtContent = document.getElementById(`modal-print-memo-subsidi-${val}`);
    console.log(prtContent);
    var WinPrint = window.open('', '', 'left=0,top=10,width=800,height=800,toolbar=0,scrollbars=0,status=0');
    WinPrint.document.write('<html><head><title>pdf</title>');
    // Tambahkan referensi ke file CSS Bootstrap di sini
  ...
  $('#basic-multiple3').select3({
	width:'100%'
  });
  $('#basic-multiple4').select4({
	width:'100%'
  });
...
<script>
	...
	function addDaging(status = 1, index = 0) {
	...
	function addDagingEdit(status, index = 0, id = -1) {
		/*const dagingCount = $('.dagings').length;
		const id = `${dagingCount}`;
		const jenisDaging = index === 0 ? 'Daging Putih' : 'Daging Merah';
	...
	function removeDagingOnClose() {
		const dagingCount = $('.dagings').length - 1;
		const id = `${dagingCount}`;
		$('#' + id).remove();
	}
	...
	function kirimDataEdit(id) {
		var tanggal = document.getElementById("tanggal-" + id).value;
		var supplier = document.getElementById("supplier-" + id).value;
		var qty = 0;
