						...
						<tr class="">
							...
							if ($datas !== null) {
								foreach ($datas as $key => $wot) {
									$wyh = $this->db->select('nama_area')->where('kode_area', $wot)->get('tbl_area')->row_array();
									if ($wyh !== null) {
										if(count($wyh) != $key ) {
											echo $wyh['nama_area'].' ';
										} else {
										...
										<div class="modal-dialog modal-lg modal-fullscreen" role="document">
											<div class="modal-content">
												...
												<div class="col-md-12">
													...
													<div class="form-group">
														<label for="password">Password</label>
														<input class="form-control mb-2" type="password" id="password" name="password" placeholder="Password" value="<?php echo $xx['password'] ?>">
													</div>
													<div class="form-group">
														<div class="col-md-12">
															<label for="area">Area</label><br>
														</div>
														<select name="wilayah[]" class="form-control col-md-12" multiple="multiple" id="basic-multiple3">
															<option value="0">Pilih Area</option>
															<?php foreach($wilayah as $wil) : ?>
																<option <?= in_array($wil['kode_area'], json_decode($xx['wilayah'], true))? 'selected' : '' ?> value="<?php echo $wil['kode_area'] ?>"><?php echo $wil['nama_area'] ?></option>
															<?php endforeach ?>
														</select>
													</div>
													<div class="innerDropdown"></div>
													...
													<div class="form-group">
														...
														<select name="role" class="form-control w-100 mb-2 col-md-12" id="basic-multiple4">
															<?php foreach ($role as $dd) : ?>
																<option <?= ($xx['role'] == $dd['id_role'])? 'selected' : '' ?> value="<?php echo $dd['id_role'] ?>"><?php echo $dd['descriptiom'] ?></option>
															<?php endforeach; ?>
														</select>
													</div>
												</div>
