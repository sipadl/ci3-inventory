...
class Main_model extends CI_Model {
	...
	left join tbl_memo b on
		a.id = b.id_sortir
	join tbl_daging c on c.id = a.id_bb where is_corection = 0
	order by a.id desc')->result_array();
	}

	public function getBahanBakuWithStatus($val) {
		return $this->db->query(
			"select td.*, ts.status as status_sortir,
			td.status as status_daging, td.keterangan as keterangan_bahan_baku,
			td.id as id_bahan_baku, ts.id as id_sortir, ts.*, b.subsidi as subsidi
			from tbl_daging td
			left join tbl_sortir ts on td.id = ts.id_bb
			left join tbl_memo b on ts.id = b.id_sortir
			where ts.status in(".$val.") and is_corection = 0 order by ts.id desc")->result_array();
	}

	public function getBahanBakuWithDate($start, $end) {
		return $this->db->query("select td.*, ts.status as status_sortir, td.status as status_daging, td.keterangan as keterangan_bahan_baku, td.id as id_bahan_baku, ts.id as id_sortir, ts.* from tbl_daging td left join tbl_sortir ts on td.id = ts.id_bb where ts.tanggal_rec >= ? and ts.tanggal_rec <= ? and ts.status >= 3 and is_corection = 0 order by ts.id desc", [$start, $end])->result_array();
	}
	...
	public function updateAll($table, $data, $id, $id_param = 'id')
	{
		$this->db->where($id_param, $id); // Assuming 'id' is the primary key column
    	$this->db->update($table, $data);
	}
	public function updateBB($table, $data, $id, $id_param = 'id')
	{
		$this->db->where($id_param, $id); // Assuming 'id' is the primary key column
    	$this->db->update($table, $data);
	}
	...
	public function delete($table, $id, $id_param = 'id') {
		$this->db->query("DELETE FROM " . $table . ' WHERE ' . $id_param . ' = ' . $id);
	...
	public function get_price_history($id_sortir) {
		return $this->db->query('select * from tbl_price_tr where id_sortir = ' . $id_sortir)->result_array();
	}
	...
	{
		return $this->db->query('select * from tbl_file order by created_at desc')->result_array();
	}
	public function kontrol2()
	{
		return $this->db->query('select * from tbl_file_2 order by created_at desc')->result_array();
	}
